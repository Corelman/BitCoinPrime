...
#if defined(NDEBUG)
# error "Bitcoinprime cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2419f7fb4ec969592141ba29a9b4f0052bcaed7f3f815db61db2938b3d1d5119");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitcoinprime: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitcoinprime Signed Message:\n";
...
{
    // Bitcoinprime: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Bitcoinprime: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Bitcoinprime: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Bitcoinprime: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitcoinprime: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitcoinprime: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x960f39e0266d3bb2c6a6495ab28b65a55778f35899eaadfb3269465bf4303a88");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BitcoinPrime Is realesed";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041802a14dd7560dfc884dd4fcc8048f7100620071edc17cdb91152517a8b5d6ba822fb7c7358ebc7fa45c3dd03f52a420686acaa4924285f16f733c3796afdbb7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525710305;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085140635;

        if (fTestNet)
        {
            block.nTime    = 1525710277;
            block.nNonce   = 385461779;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x711840070f808d27c469c50ae84c2c69d9537b59cb293140c55e22fe9d638f31"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd2 }; // Bitcoinprime: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinprimeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinprimeMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinprimeMiner(CWallet *pwallet)
{
    printf("BitcoinprimeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinprime-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinprimeMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinprimeMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinprimeMiner, pwallet));
}
